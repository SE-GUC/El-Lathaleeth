...
test("Testing 'create investor' with wrong data types", async () => {
    ...
    const newInvestor = await investor_funcs.createNewInvestor({
	    ...
	    capital: "100000",
	    capitalCurrency: "Euro"
    }).catch(error => expect(error.response.data.error).toEqual("Invalid datatype entered for one or more of the fields"));
  });

  //tests update with wrong datatype
  test("Testing 'update investor' with wrong data types",
    async() => {
        expect.assertions(1);
        const newInvestor = await investor_funcs.createNewInvestor({
            firstName: "somefirstname",
        ...
        const updatedInvestor = await investor_funcs.updateExistingInvestor({
	        ...
	        capital: "100000",
	        capitalCurrency: "Euro"
        }, newInvestor.data.data._id).catch(error => expect(error.response.data.error).toEqual("Invalid datatype entered for one or more of the fields"));
    }
);

//tests update with non existant investor id
test("Testing 'update investor' with wrong investor id",
    async() => {
        expect.assertions(1);
        const newInvestor = await investor_funcs.createNewInvestor({
        ...
        const updatedInvestor = await investor_funcs.updateExistingInvestor({
            ...
            capital: "100000",
            capitalCurrency: "Euro"
        }, newInvestor.data.data._id).catch(error => expect(error.response.data.error).toEqual("Investor does not exist"));
    }
);

//tests getByID with non existant id
test("Testing 'get by id' using wrong/non existant investor id",
    async() => {
        expect.assertions(1);
        const newInvestor = await investor_funcs.createNewInvestor({
            firstName: "somefirstname3",
        ...
        const deletedInvestor = await investor_funcs.deleteExistingInvestor(newInvestor.data.data._id);
        const getInvestor = await investor_funcs.getInvestorByID(newInvestor.data.data._id).catch(error => expect(error.response.data.error).toEqual("Investor does not exist"));
    },
    100000
);

//tests delete investor with non existant id
test("Testing 'delete investor' using wrong/non existant investor id",
    async() => {
        expect.assertions(1);
        const newInvestor = await investor_funcs.createNewInvestor({
        ...
        const deletedInvestor = await investor_funcs.deleteExistingInvestor(newInvestor.data.data._id);
        const deletedInvestor2 = await investor_funcs.deleteExistingInvestor(newInvestor.data.data._id).catch(error => expect(error.response.data.error).toEqual("Investor does not exist"));
    },
    100000
);
